...
\begin{itemize}
    \item \textbf{Prelievo dell'istruzione}: Un operazione che non mancherà mai sarà sempre il prelievo dell'istruzione, che impone che il PI non potrà mai essere nullo;
    \item \textbf{Operandi immediati}: se nel comando ci sono degli operandi immediati, allora bisognerà accedere anche altre volte alla memoria per il prelievo di tale operando. Gli accessi, per questo caso, sono dettati dalla lunghezza dell'operando rispetto ai miei bus disponibili: se lavoriamo con un'architettura a 16 bit, bisogna prelevare un dato pari a una Word, dunque il numero di accessi aggiuntivi per prelevare l'immediato è uguale a 1; se lavoriamo con un'architettyra a 32 bit (Long Word), allora il numero di accessi, a parità di architettura, sarà 2;
    \item \textbf{Variabili}: se stiamo utilizzando delle variabili, che indicano delle locazioni di memoria dirette, allora saranno necessari un numero di accessi alla memoria che permette di prelevare gli indirizzi (tali indirizzi sono lunghi tutti 32 bit). Pertanto con un architettura a 16 bit si dovranno considerare sempre 2 accessi per ogni variabile per prelevare tali indirizzi;
\end{itemize}
...
\begin{itemize}
    \item \textbf{Indirizzamento Indiretto}: quando si effettuano dei riferimenti a registri della memoria con indirizzamento indiretto allora bisogna prevedere un numero di accessi per il prelievo dell'operando. Quindi bisogna conteggiare il numero di accessi per il prelievo dell'operando in base alla sua lunghezza. Nel caso di architettura a 16 bit si avranno: 1 accesso per prelevare delle word (16 bit) e 2 accessi per prelevare le Long Word (32 bit)

    \item \textbf{Variabili}: quando si utilizzano le variabili, oltre a prelevare gli indirizzi dalla "zona istruzioni" bisogna prelevare gli operandi. La conta del numero di accessi per il prelievo degli operandi è uguale al caso di indirizzamento indiretto.
\end{itemize}
